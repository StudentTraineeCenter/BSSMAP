...
const Compass = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (
      celltowersList === null ||
      celltowersList === undefined ||
      celltowersList[0] === undefined ||
      !locationLoaded
    ) {
      setClosestMarker({
        position: {
    ...
    celltowersList.forEach((celltower, index) => {
      let currentDist = [
        Math.abs(userLoc.position.lat - celltower.lat),
        Math.abs(userLoc.position.lng - celltower.lng),
      ];
      let closestDist = [
  ...
  useEffect(() => {
    if (
      !locationLoaded ||
      closestMarker == null ||
      closestMarker.position.lat == null ||
      closestMarker.position.lng == null
    )
      return;
  ...
  const _subscribe = () => {
    setSubscription(Magnetometer.addListener(setThreeAxisData));
  };
